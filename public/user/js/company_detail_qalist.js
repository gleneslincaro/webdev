...
$(function() {
    ...
    var page_hash = location.hash.split("=");
    var num = 1;
    //$("input[name='category_list_page_num']").val();
    if (page_hash[1] != '') {
        num  = page_hash[1];
    }
    var next_page = Number(num);
    $.ajax({
        type:"POST",
        url:baseUrl+"user/joyspe_user/company_faq_ajax",
        data:{page:next_page,owner_id:owner_id,ors_id:ors_id,category_id:category_id},
        dataType: 'json'
    }).done(function(data){
        var page0 = 1;
...
//        alert('L OK!!!');
        ...
        if (1 < num) {
            ...
            $.ajax({
                ...
                $('.category_list_pager p strong').html(page0+' - '+page1);
//                $('.category_list_pager p strong').html(next_page+' - '+(next_page * data.count));
                $('.everyone_qa_category_list').html(data.html);
                $("input[name='category_list_page_num']").val(next_page);
