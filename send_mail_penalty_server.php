...
require('/var/www/html/joyspe_test_it/class.phpmailer.php');
$day = 7;
...
$condition = mysql_query("SELECT owners.email_address, owners.id AS owId, owner_recruits.id
                        ...
                        AND user_payments.user_payment_status = 5 AND DATEDIFF(NOW(),user_payments.request_money_date) > $day
...
//-----------------------------------------
    ...
    $email->sendMailBatch($data, $templ, $dataArrayUS, $templ['mail_from'], $row['email_address'], '', $str);
    $temp = $temp . $row['id'] . ",";
}
